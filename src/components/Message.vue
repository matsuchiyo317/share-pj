...
export default {
  ...
  methods: {
      ...
      if (result) {
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://nameless-caverns-04556.herokuapp.com/api/like",
              data: {
        ...
        axios
          .post("https://nameless-caverns-04556.herokuapp.com/api/like",{
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    del(index) {
      axios
        .delete(
          "https://nameless-caverns-04556.herokuapp.com/api/shares" +
        ...
        await axios
          ...
          .then((response) => {
            if (this.$route.name == "profile") {
              if (response.data.item.user.id == this.$store.state.user.id) {
